    ...
    */
    ...
    public static void DrawCurve(Vector3[] allPoints, Vector2[] allNormals, Color[] allColors)
    {
        if (Event.current.type != EventType.repaint)
            return;
        //Debug.Log("ccccc "+ allPoints.Length);
        CreateMaterial();
        LineMaterial.SetPass(0);
   //     Debug.Log("Pos=" + allPoints[0] + "  " + allPoints[1]);
   //     Debug.Log("allNormal=" + allNormals[0] + "  " + allNormals[1]);
    ...
    /// <summary>
    /// 绘制两个点之间的贝塞尔曲线 曲线的两个点的切线分别为X轴方向上一段 TangentLength 偏移
    /// </summary>
    ...
    /// <returns></returns>
    public static Rect DrawBezier(Vector2 start,  Vector2 end, float TangentLength, Color color, float width)
    {
        Vector2 startTangent = start + new Vector2(-1 * TangentLength, 0);
        Vector2 endTangent = end + new Vector2( -1*TangentLength, 0);

        return DrawBezier(start, startTangent, end,endTangent,color,width);
    }
