...
namespace Csv
{
    public class CsvUtil
    {
        ...
        public static CsvReader LoadReader(string path)
        {
            ...
            if (a == null)
            {
                Debug.LogError(string.Format("找不到{0}.csv", path));
                return null;
            }
        ...
        public static List<T> Load<T>(string csvName) where T : new()
        {
            ...
            if (Fields.Count == 0)
            {
                Debug.LogError(string.Format("{0}.csv解析不到列名", csvName));
                return l;
            }
            ...
            foreach(string f in Fields){
                if(!FieldInfos.ContainsKey(f))
                    Debug.LogError(string.Format("{0}.csv 不包含列名:{1}", csvName, f));
            }
            ...
            while (r.Read())
            {
                ...
                if (StringUtil.TryParse(val, fieldInfo.FieldType, out outVal))
                    ...
                    Debug.LogError(string.Format("{0}.csv解析出错  行:{1} 列:{2} 列名:{3} ", csvName, r.Row, r.Col, Fields[r.Col]));
            }
        ...
        public static Dictionary<TKeyType, T> Load< TKeyType,T>(string csvName,string key,Func<T,string,CsvReader,string,bool> onParseEx= null) where T : new()
        {
            ...
            if (keyField == null)
            {
                Debug.LogError(string.Format("{0}.csv 类找不到对应的键值字段:{1}", csvName, key));
                return l;
            }
            if (!keyField.FieldType.Equals(typeof(TKeyType)))
            {
                Debug.LogError(string.Format("{0}.csv 键值字段的类型和Dictionary不一致:{1}", csvName, key));
                return l;
            }
            if (!Fields.Contains(key))
            {
                Debug.LogError(string.Format("{0}.csv 不包含键值列:{1} 是否被删除？", csvName, key));
                return l;
            }
            ...
            while (r.Read())
            {
                ...
                if (endOfRow)
                {
                    ...
                    {
                        if(l.ContainsKey(k))
                             Debug.LogError(string.Format("{0}.csv 行:{1} 键值({2})重复 是不是没有填 ", csvName, r.Row,key));
                        l[k]= row;
                    }
                    row = new T();
                }
        ...
        public static bool Load(string path, out List<Dictionary<string, string>> l, out List<string> fields, out List<string> descs)
        {
            ...
            if(Fields.Count == 0)
            {
                Debug.LogError(string.Format("{0}.csv解析不到列名", path));
                return false;
            }
            ...
            while (r.Read())
            {
                ...
                {
                    Debug.LogError(string.Format("{0}.csv解析出错 字段名重复", path, Fields[r.Col]));
                    continue;
                }
            ...
            if(r.IsError)
                Debug.LogError(string.Format("{0}.csv解析出错 行:{1} 列:{2}", path, r.Row,r.Col));
