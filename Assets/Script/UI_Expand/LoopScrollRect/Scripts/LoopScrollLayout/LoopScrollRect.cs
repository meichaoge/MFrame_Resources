...
namespace UnityEngine.UI
{
    ...
    public abstract class LoopScrollRect : UIBehaviour, IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup
    {
        ...
        [Header("Reverse direction for dragging")]
        ...
        {
            if (prefabSource != null && prefabSource.m_ItemPrefab != null)
                return GetSize(prefabSource.m_ItemPrefab.transform as RectTransform);
            return 1;
        }
        ...
        public float horizontalNormalizedPosition
        {
            get
            {
                ...
                if (m_TotalCount > 0 && itemTypeEnd > itemTypeStart)
                {
                    ...
                    float totalSize = elementSize * m_TotalCount;
        ...
        [SerializeField]
        protected bool m_NeedUpdate = false; //标识是否需要刷新 (避免不必要的刷新)


        public Action<RectTransform> OnItemRemoveEvent;  //列表象被移除
        public Action<RectTransform,int> OnItemCreateEvent; //列表象被创建

        public readonly List<RectTransform> m_AllItems = new List<RectTransform>();   //所有创建的列表项目
        #endregion
        ...
        protected virtual void LateUpdate()
        {
            ...
            {
                //Debug.Log("AAA "+ m_Velocity+ "      Time.unscaledDeltaTime="+ Time.unscaledDeltaTime);
        ...
        private void ReturnObjectAndSendMessage(Transform go)
        {
            ...
            m_AllItems.Remove(go as RectTransform);

            if (OnItemRemoveEvent != null)
                OnItemRemoveEvent(go as RectTransform);
        ...
        /// 初始化视图并填充数据
        /// </summary>
        /// <param name="dataCount">总的数据项个数</param>
        ...
        public void RefillCells(int dataCount, int offset = 0)
        {
            ...
            m_NeedUpdate = true;
            StopMovement();
            itemTypeStart = reverseDirection ? m_TotalCount - offset : offset;
        ...
        public void RefillCellsFromEnd(int offset = 0)
        {
            ...
            if (!Application.isPlaying || m_TotalCount < 0 || m_ContentConstraintCount > 1 || prefabSource == null)
                return;

            StopMovement();
            itemTypeEnd = reverseDirection ? offset : m_TotalCount - offset;
        ...
        {
            if (m_TotalCount >= 0 && itemTypeStart - m_ContentConstraintCount < 0)
                return 0;
        ...
        protected float NewItemAtEnd()
        {
            ...
            int count = m_ContentConstraintCount - (content.childCount % m_ContentConstraintCount);
        ...
        {
            if (((m_Dragging || m_Velocity != Vector2.zero) && m_TotalCount >= 0 && itemTypeStart < m_ContentConstraintCount)
                || content.childCount == 0)
            {
                return 0;
            }
            ...
            for (int i = 0; i < m_ContentConstraintCount; i++)
            {
                ...
                itemTypeEnd--;
                if (itemTypeEnd % m_ContentConstraintCount == 0 || content.childCount == 0)
        ...
        {
            //Debug.Log("InstantiateNextItem  "+ prefabSource);
            ...
            m_AllItems.Add(nextItem);

            if (OnItemCreateEvent != null)
                OnItemCreateEvent(nextItem, itemIdx);
            return nextItem;
        }
        ...
        public virtual void SetContentAnchoredPosition(Vector2 position)
        {
            ...
            //Debug.Log("SetContentAnchoredPosition "+position +"     ::"+ m_ViewBounds);
        ...
        /// <summary>
        /// 2017/12 /20 新增  拖拽后的阻尼运动(惯性)
        /// </summary>
        protected virtual void DampingMoving()
        {
            if (m_Dragging) return;
       //       Debug.Log("DampingMoving  "+ offset+ "      m_Velocity=" + m_Velocity);
            if (offset != Vector2.zero || m_Velocity != Vector2.zero)
            {
              //  Debug.Log("AAAAAAAAAAAA" + offset+ "          m_Velocity="+ m_Velocity);
                ...
                for (int axis = 0; axis < 2; axis++)
                {
                    ...
                    if (m_MovementType == MovementType.Elastic && offset[axis] != 0)
                    {
                        ...
                        position[axis] = Mathf.SmoothDamp(m_Content.anchoredPosition[axis], m_Content.anchoredPosition[axis] + offset[axis], ref speed, 0, Mathf.Infinity, Time.unscaledDeltaTime);
                   ...
                   //     Debug.Log("DampingMoving m_Velocity=" + m_Velocity+ "  position="+ position);
                    }
        ...
        private void UpdateScrollbars(Vector2 offset)
        {
            if (m_HorizontalScrollbar)
            {
                ...
                {
                    m_HorizontalScrollbar.size = Mathf.Clamp01((m_ViewBounds.size.x - Mathf.Abs(offset.x)) / m_ContentBounds.size.x * (itemTypeEnd - itemTypeStart) / m_TotalCount);
                }
                else
            ...
            if (m_VerticalScrollbar)
            {
                ...
                {
                    m_VerticalScrollbar.size = Mathf.Clamp01((m_ViewBounds.size.y - Mathf.Abs(offset.y)) / m_ContentBounds.size.y * (itemTypeEnd - itemTypeStart) / m_TotalCount);
                }
                else
        ...
        private Vector2 CalculateOffset(Vector2 delta)
        {
            ...
            //Debug.Log("  min=" + min + "  max=" + max + "  m_ViewBounds.min" + m_ViewBounds.min + "   max=" + m_ViewBounds.max+ "   delta="+ delta);
            ...
            if (m_Horizontal)
            {
                ...
                {
                    //Debug.Log("aaa " + max + "   min=" + min + "  m_ViewBounds.max=" + m_ViewBounds.max + " m_ViewBounds.min=" + m_ViewBounds.min);
                ...
                else if (max.x <= m_ViewBounds.max.x)
                {
                    if (max.x < 0)
                    {
                        offset.x = m_ViewBounds.center.x - m_Content.anchoredPosition.x;
                  //      Debug.Log("ccc offset=" + offset+ "  m_ViewBounds"+ m_ViewBounds.min);
                    }
                    else
                    {
                        offset.x = m_ViewBounds.max.x - max.x;
                    //    Debug.Log("bbb " + max + "   min=" + min + "  m_ViewBounds.max=" + m_ViewBounds.max + " m_ViewBounds.min=" + m_ViewBounds.min + "  offset=" + offset);
                    }
                }
                else
                {
          ...
          //  Debug.Log("CalculateOffset  delta=" + delta+ "            offset="+ offset);
        ...
        public virtual void OnScroll(PointerEventData data)
        {
            ...
            m_NeedUpdate = true;
            //   Debug.Log("OnScroll  AAAAAAAAAAAAAAAAAAAAAAAA");
