...
{
    public class Simple_Cube_MeshLineTool_Parent : BaseMeshTool
    {
        protected override void CreateSubMesh<Simple_Cube_SubMesh>()
        {
            if (m_DrawMeshState == DrawMeshState.Begin) return;
            base.CreateSubMesh<Simple_Cube_SubMesh>();
        ...
        private void Update()
        {
            if (Input.GetKeyDown(KeyCode.R))
            {
                ...
                //Debug.Log("AA " + point);
                CreateSubMesh<Simple_Cube_SubMesh>();
                AddPoint(point);
            }
