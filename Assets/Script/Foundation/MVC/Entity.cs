...
public abstract class Entity
{
    ...
    public void AddListener<T, U, V,W>(Enum p_type, Callback<T, U, V,W> pListener)
    {
        _table[p_type] = _table.ContainsKey(p_type) ? Delegate.Combine(_table[p_type], pListener) : pListener;
    }
    ...
    public void RemoveListener<T, TU, TV,TW>(Enum pType, Callback<T, TU, TV,TW> pListener)
    {
        if (_table.ContainsKey(pType))
        {
            Delegate d = Delegate.Remove(_table[pType], pListener);
    ...
    public void Dispatch<T, TU, TV,TW>(Enum pType, T arg1, TU arg2, TV arg3,TW arg4)
    {
        if (!_table.ContainsKey(pType)) return;
        ...
        if (delega != null)
        {
            var callback = delega as Callback<T, TU, TV,TW>;
